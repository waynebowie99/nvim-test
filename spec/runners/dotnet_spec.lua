...
describe("dotnet", function()
    ...
    it("run file", function()
        ...
        assert.are.same({ "dotnet", "test", "--filter", "FullyQualifiedName~NameSpace.Fixtures.Tests" },
    ...
    it("run nearest function", function()
        ...
        assert.are.same(
            { "dotnet", "test", "--filter", "FullyQualifiedName=Namespace.Fixtures.Tests" },
            vim.g.test_latest.cmd
        )
        helpers.view(filename, 8)
        vim.api.nvim_command "TestNearest"
        assert.are.same(
            { "dotnet", "test", "--filter", "FullyQualifiedName=Namespace.Fixtures.Tests.Test" },
            vim.g.test_latest.cmd
        )
    end)
